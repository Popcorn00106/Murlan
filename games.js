...
const reelSymbols = ['$', '7', '*', '🍒', '🔔']; // Expanded symbols for the slot machine reels
const questions = [
    {
...
function checkWin(reels) {
    ...
    if (winnings > 0) {
        ...
        highlightWinningReels(reels, symbolCount);
    } else {
...
function highlightWinningReels(reels, symbolCount) {
    reels.forEach((symbol, index) => {
        if (symbolCount[symbol] >= 3) {
            document.getElementById(`reel-${index + 1}`).classList.add('win');
        }
    });
}
